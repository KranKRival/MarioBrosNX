...
static void WriteLogFile(const char* path, LogType type, const char *fmt, ...)
{
...
CCore::CCore(void) {
	...
	window = SDL_CreateWindow("MarioBrosNX", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, CCFG::GAME_WIDTH, CCFG::GAME_HEIGHT, SDL_WINDOW_SHOWN);
...
void CCore::InputMenu() {
    ...
    switch (mainEvent->type) 
	{
		...
		WriteLogFile("sdmc://WriteLogFile.log", LogType_Info,"Joystick %d axis %d value: %d\n", mainEvent->jaxis.which, mainEvent->jaxis.axis, mainEvent->jaxis.value);
        break;

        case SDL_JOYBUTTONDOWN:
        //WriteLogFile("sdmc://WriteLogFile.log", LogType_Info, "Joystick %d button %d down\n", mainEvent->jbutton.which, mainEvent->jbutton.button);
		switch (mainEvent->jbutton.button) 
		{
			case 15: // Key (Dpad Down)
			if(!keyMenuPressed) 
			{
...
void CCore::InputPlayer() {
	...
	{
		if(mainEvent->jbutton.button == 14) // Key (Dpad Right) 
		{
				if(firstDir) 
				{
					firstDir = false;
		...
		if(mainEvent->jbutton.button == 15) // Key (Dpad Down)
		{
			oMap->getPlayer()->setSquat(false);
			keyS = false;
		}
		
		if(mainEvent->jbutton.button == 12)  // Key (Dpad Left)
		{
			if(!firstDir) 
			{
				firstDir = true;
		...
		if(mainEvent->jbutton.button == 0) // Key (A)
		{
			CCFG::keySpace = false;
	    }
		
		if(mainEvent->jbutton.button == 1) // Key (B)
		{
			if(keyShift) 
			{
